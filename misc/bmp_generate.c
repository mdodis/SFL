...
static const Generator The_Generators[] = {
    {gen_v5_1bpp, "v5-1bpp", "Generates v5 bitmap with 1 bit per pixel"},
    {gen_v1_16bpp_rgb,
     "v1-16bpp-rgb",
...
static void print_help()
{
    ...
    for (int i = 0; i < ARRAY_COUNT(The_Generators); ++i) {
        const Generator* generator = &The_Generators[i];
        printf("%-20s %s\n", generator->tag, generator->description);
    }
}

static const Generator* find_generator(const char* tag)
{
    for (int i = 0; i < ARRAY_COUNT(The_Generators); ++i) {
        const Generator* generator = &The_Generators[i];
...
int main(int argc, char const* argv[])
{
    ...
    const Generator* generator = find_generator(argv[1]);
    if (generator == 0) {
...
static PROC_GENERATE(gen_v5_1bpp)
{
    ...
    ADD_BYTES(f, 0b10000000, 0x00, 0x00, 0x00, 0b01000000, 0x00, 0x00, 0x00);
}
...
static PROC_GENERATE(gen_v1_16bpp_rgb)
{
    ...
    // Image data (54 bytes)
    ADD_BYTES(f, 0xe0, 0x03, 0xff, 0x7f, 0x00, 0x7c, 0x1f, 0x00);
}
